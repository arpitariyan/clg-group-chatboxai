...
export async function checkUserPlan(userEmail) {
    ...
    try {
        ...
        // The supabase client is configured with a 5s fetch timeout that resolves
        // AbortErrors as { error: { code: 'TIMEOUT' } } instead of throwing.
        ...
        if (userError && userError.code !== 'PGRST116') {
            if (userError.code === 'TIMEOUT' || userError.message?.includes('timeout') || userError.message?.includes('fetch failed')) {
                // Supabase is unreachable - return a safe default
                // For the known permanent account, still grant pro access
                const isPermanentAccount = userEmail === 'arpitariyanm@gmail.com';
                console.warn('Supabase unreachable in checkUserPlan, using fallback for:', userEmail);
                return {
...
export async function getDailyImageCount(userEmail) {
    ...
    try {
        ...
        if (error) {
            if (error.code === 'TIMEOUT' || error.message?.includes('timeout') || error.message?.includes('fetch failed')) {
                console.warn('Supabase unreachable in getDailyImageCount, defaulting to 0');
                return 0;
            }
...
export async function getMonthlyResearchCount(userEmail) {
    ...
    try {
        ...
        if (error) {
            if (error.code === 'TIMEOUT' || error.message?.includes('timeout') || error.message?.includes('fetch failed')) {
                console.warn('Supabase unreachable in getMonthlyResearchCount, defaulting to 0');
                return 0;
            }
