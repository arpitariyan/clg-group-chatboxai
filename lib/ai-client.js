 ...
 */
function normalizeWebsiteCodeForComparison(code = '') {
...
REMEMBER: Make the requested changes while maintaining the high quality of the original website. Return ONLY the complete updated HTML code, nothing else.`;
    ...
    const conversationContext = recentConversation
        ...
        : '';
    const normalizedCurrentCode = normalizeWebsiteCodeForComparison(currentCode);
            ...
            try {
                ...
                const response = await client.chat.completions.create({
                    ...
                    messages: [
                        ...
                        {
                            role: 'user',
                            content: `Current code:\n${currentCode}\n\n${conversationContext}Latest user request:\n${revisionRequest}\n\nUpdate the website so the requested changes are clearly reflected in the returned HTML while preserving the rest of the site.`
                        }
                    ],
                    temperature: 0.7,
                    max_tokens: 8000,
                ...
                if (normalizeWebsiteCodeForComparison(code) === normalizedCurrentCode) {
                    console.warn(`⚠️ [Code Revision] ${candidateModel} attempt ${attempt} returned unchanged code.`);
                    await waitBeforeRetry(attempt);
                    continue;
                }
